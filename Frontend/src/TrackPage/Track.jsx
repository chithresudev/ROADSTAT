...
function TrackPage({ updateHeader, updateButton }) {
  ...
  useEffect(() => {
    if (searchedTruckNo) {
      const foundIndex = trackData.findIndex(
        (trackLocation) => trackLocation.truckId === searchedTruckNo
      );
      if (foundIndex !== -1) {
  ...
  const fetchTruckLocationsAndDestinations = async (truckId) => {
    try {
      const locationsResponse = await fetch(
        `${apiUrl}/truck-location/${truckId}`
      );
      const destinationsResponse = await fetch(
        `${apiUrl}/destinations/${truckId}`
      );
  ...
  const filteredTrackData = showRunningOnly
    ? trackData.filter((item) => item.locationStatus === "Running")
    : trackData;
  const searchedTruckIndex = filteredTrackData.findIndex(
    (trackLocation) => trackLocation.truckId === searchedTruckNo
  );
  ...
  useEffect(() => {
    if (searchedTruckNo) {
      const foundIndex = sortedTrackData.findIndex(
        (trackLocation) => trackLocation.truckId === searchedTruckNo
      );
      if (foundIndex !== -1) {
        setSelectedRowIndex(foundIndex);
  ...
  return (
    <div className="track_page">
      <div className="track-card">
        <table className="track-table table_content">
          <thead>
            <tr>
              <th>S No</th>
              <th>Vehicle No</th>
      ...
      <div className="track-below">
        <div className="track-mainprofile">
          ...
          <div className="track-details">
            <button className="track-map" onClick={handleOpenMaps}>
              Open with Maps
            </button>
          </div>
        </div>
                ...
                <table className="track-table">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <Link
                          to={`/driver?driverId=${truckDestinations.driverId}`}
                          className={
                            truckDestinations.driverId ? "click" : "clickempty"
                          }
                        >
                          {truckDestinations.driverId
                            ? truckDestinations.driverId
                            : "-"}
                        </Link>
                      </td>
                    ...
                    <tr>
                      ...
                      <td>
                        {truckDestinations.destinationName
                          ? truckDestinations.destinationName
                          : "-"}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        <Link
                          to={`/maintenance?truckNo=${truckDestinations.truckId}`}
                          className="click"
                        >
                          Click Here
                        </Link>
